...
class BaseDatos:
    ...
    def registrar_cliente(self, nombres: str, apellidos: str) -> Cliente:
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT id FROM clientes WHERE LOWER(nombres) = ? AND LOWER(apellidos) = ?",
                      (nombres.lower(), apellidos.lower()))
        if cursor.fetchone():
            raise ValueError("El cliente ya existe.")
        
        cid = self._nuevo_id("C")
        cursor.execute("INSERT INTO clientes (id, nombres, apellidos) VALUES (?, ?, ?)",
                      (cid, nombres, apellidos))
        self.conn.commit()
    ...
    def registrar_sala(self, nombre: str, cupo: int) -> Sala:
        ...
        cursor.execute("INSERT INTO salas (id, nombre, cupo) VALUES (?, ?, ?)",
                      (sid, nombre, cupo))
        self.conn.commit()
    ...
    def salas_disponibles(self, fecha_dt: datetime, turno: str) -> List[Sala]:
        ...
        cursor.execute("""
            ...
            WHERE s.id NOT IN (
                ...
                WHERE DATE(fecha) = :fecha AND turno = :turno
            )
        """, {"fecha": fecha_buscar, "turno": turno})
        return [Sala(*row) for row in cursor.fetchall()]

    def registrar_reserva(self, evento: str, id_cliente: str, id_sala: str, 
                         fecha_dt: datetime, turno: str) -> Reservacion:
        ...
        cursor.execute("""
            ...
            WHERE id_sala = :sala AND DATE(fecha) = :fecha AND turno = :turno
        ...
        cursor.execute("""
            INSERT INTO reservaciones (folio, evento, id_cliente, id_sala, fecha, turno)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (rid, evento, id_cliente, id_sala, fecha_dt, turno))
        self.conn.commit()
        return Reservacion(folio=rid, evento=evento, id_cliente=id_cliente, 
                          id_sala=id_sala, fecha=fecha_dt, turno=turno)
    ...
    def reservas_en_rango(self, desde_dt: datetime, hasta_dt: datetime) -> List[Reservacion]:
        ...
        cursor.execute("""
            SELECT folio, evento, id_cliente, id_sala, fecha, turno
            FROM reservaciones
    ...
    def editar_nombre_evento(self, folio: str, nuevo_nombre: str) -> Reservacion:
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT folio FROM reservaciones WHERE folio = ?", (folio,))
        if not cursor.fetchone():
        ...
        cursor.execute("SELECT folio, evento, id_cliente, id_sala, fecha, turno FROM reservaciones WHERE folio = ?", (folio,))
...
def opcion_editar_evento(db: BaseDatos):
    ...
    try:
        ...
        print(linea())
        filas = [[r.folio, r.evento, fecha_a_str(r.fecha)] for r in reservas]
...
def opcion_consultar_por_fecha(db: BaseDatos):
    ...
    if export_op in {"1", "2", "3"}:
        nombre_base = f"reporte_{fecha_a_str(fecha_dt).replace('-', '')}"
        try:
            if export_op == "1":
...
def menu():
    ...
    opciones = {
        ...
        "4": ("Registrar a un nuevo cliente", opcion_registrar_cliente),
        "5": ("Registrar una sala", opcion_registrar_sala),
        "6": ("Salir", None),
    }
